...
describe('Vector2Int dot product', () => {
  test.each([
    ...
    [[2, 5], [5, 3], 25],
    [[-1, -10], [1, -5], 49],
  ])('Dot product of %s and %s is %p', (a, b, result) => {
...
describe('Vector2Int addition', () => {
  const inputs = [
    [new Vector2Int(1, 2), new Vector2Int(2, 1), new Vector2Int(3, 3)],
    [Vector2Int.origin, Vector2Int.one, Vector2Int.one],
  ];
...
describe('Vector2Int subtraction', () => {
  const inputs = [
    [Vector2Int.origin, Vector2Int.right, Vector2Int.left],
    [new Vector2Int(10, 100), Vector2Int.one, new Vector2Int(9, 99)],
  ];
...
describe('Vector2Int scale', () => {
  const inputs = [
    [Vector2Int.origin, 3, Vector2Int.origin],
    [Vector2Int.one, 3, new Vector2Int(3, 3)],
    [Vector2Int.one, -1, new Vector2Int(-1, -1)],
    [Vector2Int.one, 0, Vector2Int.origin],
  ];
...
describe('Vector2Int metrics', () => {
  test.each([
    [55, 21, 76],
    [32, 54, 86],
  ])('manhattan length of (%p, %p) is %p', (x, y, length) => {
  ...
  test.each([
    [[1, 1], [2, 2], 2],
    [[5, 64], [56, 32], 83],
  ])('distance between %p and %p is %p', (a, b, length) => {
    const pointA = new Vector2Int(a[0], a[1]);
    const pointB = new Vector2Int(b[0], b[1]);
...
describe('Vector2Int spatial queries', () => {
  test.each([
    [[0, 0], [2, 2], [1, 2], true],
    [[0, 0], [1, 2], [5, 2], false],
    [[-10, -5], [1, 10], [-1, -10], false],
  ])('does %s and %s contain %s: %p', (min, max, point, result) => {
  ...
  ])('bounds %s and %s clamp %s to %s', (min, max, point, result) => {
    const minBound = new Vector2Int(min[0], min[1]);
    const maxBound = new Vector2Int(max[0], max[1]);
    const vector = new Vector2Int(point[0], point[1]);
